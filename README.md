...
Two transports, picked automatically — see [DEPLOY.md](DEPLOY.md):

- **firecrawl CLI** locally, so searches run on your authenticated plan.
- **Firecrawl REST API** when `FIRECRAWL_API_KEY` is set, or whenever the CLI is
  missing. This is the only one that works on hosted boxes such as Streamlit
  Cloud, where a Node CLI cannot be installed. With no key it still runs, just
  usage-limited.
