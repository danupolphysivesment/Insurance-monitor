...
def cli_status() -> tuple[bool, str]:
    """(ok, message) — for the dashboard to show before a scan is attempted."""
    try:
        return True, firecrawl_bin()
...
def _run_firecrawl(query: str, limit: int, country: str, tbs: str,
    ...
    for attempt in range(retries):
        ...
        # Kept short on purpose: a stalled worker shows up as a frozen progress
        # bar, and a search that is still 429ing after ~15s is better reported
        # than waited on.
...
def collect(platforms: Iterable[str], queries: Iterable[str],
    ...
    """
    firecrawl_bin()   # raises once, with a fix, instead of failing per query
